...
CVAPI(CvVec2i) core_FileNode_read_Vec2i(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec2i v; (*node) >> v; CvVec2i ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec3i) core_FileNode_read_Vec3i(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec3i v; (*node) >> v; CvVec3i ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec4i) core_FileNode_read_Vec4i(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec4i v; (*node) >> v; CvVec4i ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec6i) core_FileNode_read_Vec6i(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec6i v; (*node) >> v; CvVec6i ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec2d) core_FileNode_read_Vec2d(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec2d v; (*node) >> v; CvVec2d ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec3d) core_FileNode_read_Vec3d(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec3d v; (*node) >> v; CvVec3d ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec4d) core_FileNode_read_Vec4d(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec4d v; (*node) >> v; CvVec4d ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec6d) core_FileNode_read_Vec6d(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec6d v; (*node) >> v; CvVec6d ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec2f) core_FileNode_read_Vec2f(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec2f v; (*node) >> v; CvVec2f ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec3f) core_FileNode_read_Vec3f(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec3f v; (*node) >> v; CvVec3f ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec4f) core_FileNode_read_Vec4f(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec4f v; (*node) >> v; CvVec4f ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec6f) core_FileNode_read_Vec6f(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec6f v; (*node) >> v; CvVec6f ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec2b) core_FileNode_read_Vec2b(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec2b v; (*node) >> v; CvVec2b ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec3b) core_FileNode_read_Vec3b(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec3b v; (*node) >> v; CvVec3b ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec4b) core_FileNode_read_Vec4b(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec4b v; (*node) >> v; CvVec4b ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec6b) core_FileNode_read_Vec6b(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec6b v; (*node) >> v; CvVec6b ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec2s) core_FileNode_read_Vec2s(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec2s v; (*node) >> v; CvVec2s ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec3s) core_FileNode_read_Vec3s(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec3s v; (*node) >> v; CvVec3s ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec4s) core_FileNode_read_Vec4s(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec4s v; (*node) >> v; CvVec4s ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec6s) core_FileNode_read_Vec6s(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec6s v; (*node) >> v; CvVec6s ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec2w) core_FileNode_read_Vec2w(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec2w v; (*node) >> v; CvVec2w ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec3w) core_FileNode_read_Vec3w(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec3w v; (*node) >> v; CvVec3w ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec4w) core_FileNode_read_Vec4w(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec4w v; (*node) >> v; CvVec4w ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
CVAPI(CvVec6w) core_FileNode_read_Vec6w(cv::FileNode *node) {     EXC_SAFE((
    cv::Vec6w v; (*node) >> v; CvVec6w ret; std::copy(std::begin(v.val), std::end(v.val), std::begin(ret.val)); return ret;
    )) }
